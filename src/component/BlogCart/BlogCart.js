...
const BlogCart = ({ blogs }) => {
    ...
    const sliceText = blogs.content.slice(0, 180);

    return (
        <>
            <div onClick={handleClick} class="m-10 cursor-pointer mx-auto bg-gray-200">
                <div id='blogCart' class="relative  rounded-lg block md:flex items-center bg-gray-100 shadow-xl" style={{ minHeight: '19rem' }}>
                    <div class="relative w-full md:w-2/5 h-full overflow-hidden rounded-t-lg md:rounded-t-none md:rounded-l-lg" style={{ minHeight: '19rem' }}>
                        <img class="absolute inset-0 w-full h-full object-cover object-center" src={`data:image/png;base64,${blogs.image.img}`} alt="" />
                    ...
                    <div class="w-full md:w-3/5 h-full flex items-center bg-gray-100 rounded-lg">
                        <div class="p-12 md:pr-12 md:pl-10 md:py-10">
                            <p style={{ color: '#f06a35' }} >{blogs.title}</p>
                            <p class="text-gray-600"> {sliceText}</p>
                            <a class="flex items-baseline mt-3 text-indigo-600 hover:text-indigo-900 focus:text-indigo-900" href="#">
                                <span>Read the Full Blog :{blogs.id}</span>
